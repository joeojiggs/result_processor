...
if (isset($_POST['grade'])){
        ...
        if (mysqli_num_rows($wuery) > 0){
            ...
            while ($low = mysqli_fetch_array($wuery)){
                ...
                $avg = $grand_total/3;
                 ...
                 $sql4 = "update ".$class_name."_".$sub_id." set `grand_total` = '$grand_total', `grand_avg` = '$avg' where Reg_Num = '$reg_no'";
    ...
    // NOTE: varable olodo use to prevent 0 coming out first and variable last_total and last_i use to grade bracket positions the right way
    $olodo = 0;
    $i=1;
    $last_total = "";
    $last_i;
      ...
      <?php
          ...
          while($row = mysqli_fetch_array($query)) {
            ?> 
               ...
               <td>
               <?php
               $query2 = mysqli_query($con2,"select * from ".$class_name."_".$sub_id." where reg_no =".$row['reg_no']);
               $row2 = mysqli_fetch_array($query2); 
               echo $row2['Total']; 
               ?>
               </td>
               <td>
               <?php
               $query2 = mysqli_query($con3,"select * from ".$class_name."_".$sub_id." where reg_no =".$row['reg_no']);
               $row2 = mysqli_fetch_array($query2); 
               echo $row2['Total']; 
               ?>
               </td>
